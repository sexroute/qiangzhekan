...
@synthesize _animationOrder;
@synthesize _tileButtons;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
		...
		// Clockwise from left.
        self._animationOrder = [NSMutableArray arrayWithObjects:
...
- (void)loadView
{
	...
	// Tiles
	self._tileButtons = [NSMutableArray arrayWithCapacity:6];
	...
	if (_singlePageMaxTiles) {
		numTiles++;
		[self._animationOrder insertObject:[NSNumber numberWithInteger:5] atIndex:0];
	}
	for (int i = 0; i < (numTiles + 1); i++) {
		...
		if (i == numTiles) {
			...
			j = [[self._animationOrder objectAtIndex:i] integerValue];
			if (_rightHanded) {
				tileButton.layer.zPosition = (numTiles - [self._animationOrder indexOfObject:[NSNumber numberWithInt:i]]);
			} else {
				tileButton.layer.zPosition = [self._animationOrder indexOfObject:[NSNumber numberWithInt:i]];
			}
			...
			[self._tileButtons addObject:tileButton];
		}
...
- (void)viewDidUnload
{
	...
	self._animationOrder = nil;
	_closeButton = nil;
	_pageButton = nil;
	self._tileButtons = nil;
}
...
- (CGPoint)displayMenuPage:(NSInteger)pageNum centeredOnPoint:(CGPoint)centerPt inView:(UIView *)parentView
{
	...
	// Position tiles.
	for (UIButton *tileButton in self._tileButtons) {
		tileButton.frame = [self frameForCenteredTile];
	...
	if (_singlePageMaxTiles && self._animationOrder.count == MG_TILES_PER_PAGE) {
		[self._animationOrder insertObject:[NSNumber numberWithInteger:5] atIndex:0];
	}
...
- (void)animationDidStop:(CAAnimation *)animation finished:(BOOL)finished
{
	...
	if (animation) {
		...
		if ([name isEqualToString:MG_ANIMATION_TILES]) {
			...
			NSInteger lastAnimatedTileIndex = [[self._animationOrder lastObject] integerValue];
			if (_tilesArranged) {
...
- (void)animateTilesForCurrentPage
{
	...
	NSInteger j;
	NSInteger numAnimatedTiles = self._tileButtons.count;
	for (int i = 0; i < numAnimatedTiles; i++) {
		j = [[self._animationOrder objectAtIndex:i] integerValue];
		if (j >= [self._tileButtons count]) {
			break;
		}
		tile = [self._tileButtons objectAtIndex:j];
